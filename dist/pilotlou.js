...
class Rect {
    ...
    draw(ctx) {
        ...
        ctx.fillRect(this.position.x - this.dimensions.x / 2, this.position.y - this.dimensions.y / 2, this.dimensions.x, this.dimensions.y);
    }
    collidesWith(other) {
...
class Enemy extends Rect {
    constructor(position, dimensions, direction) {
...
class Game {
    ...
    get elapsedTime() {
        return ((this.state === "playing" ? new Date().getTime() : this.pauseTime) -
            this.startTime);
    }
    get dimensions() {
    ...
    newGameState() {
        this.player = new Player({ x: this.dimensions.x / 2, y: this.dimensions.y / 2 }, { x: 40, y: 40 });
        this.enemies = [
...
window.onload = setup;
//# sourceMappingURL=pilotlou.js.map